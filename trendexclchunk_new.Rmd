...
```{r echo=FALSE}
    ...
    pTest = function(x) {
  t=prop.test(sum(x, na.rm=TRUE), length(!is.na(x)))
  est = round(c(t$estimate, t$conf.int)*100, 3)
  est
}
yy=matrix(unlist(tapply(trendexcl$NumNRTISDRMs, trendexcl$IsolateYear, pTest)), ncol=3,byrow=TRUE)
colnames(yy) = c("est2", "lb2", "ub2")
df = cbind(df, yy)
```
